...
class PouchStore extends Store {
  ...
  override def loadDrawings: Future[Seq[Seq[Position]]] = {
    db.get(Key.drawings.toString)
      .map { json =>
        val doc = json.asInstanceOf[Drawings with PouchDoc]
        drawingsRev = Some(doc._rev)
        doc.polygons.toSeq.map(_.toSeq.map(ll => Position(ll.lat, ll.lng)))
      }
      .recover { case t => Seq() }
  }

  override def saveDrawings(featureGroup: Seq[Seq[Position]]): Future[Unit] =
    db.put(
      new Drawings {
